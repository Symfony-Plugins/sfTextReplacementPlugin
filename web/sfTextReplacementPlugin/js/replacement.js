function sfTextReplacement_single(selector, url, wordwrap)
{
  var replacements=new Array();
  replacements[0] = {selector: selector, wordwrap: wordwrap};
  com_stewartspeak_replacement(replacements, url);
}

function com_stewartspeak_replacement(replacements, sfTextReplacementUrl) {
/*
...
{
	replaceSelector(replacements[i].selector,sfTextReplacementUrl, replacements[i].wordwrap);
}
var testURL = "/sfTextReplacementPlugin/images/1x1.gif" ;
...
function replacement()
{
	for(var i=0;i<items.length;i++)
	{
		...
		if(elements.length > 0) for(var j=0;j<elements.length;j++)
		{
			...
			{
				var url = items[i].url + escape(items[i].selector) + '/' + escape(tokens[k]+' ')+'.png';
